...
def labelPCGStates(envelope, s1_positions, s2_positions, samplingFrequency):
    ...
    states = np.zeros((envelope.shape[0]))
    ...
    for i in range(s1_positions.shape[0]):
        upper_bound = np.round(np.min(states.shape[0]), s1_positions[i] + mean_S1)

        # MIGHT BE AN OFF BY ONE ERROR HERE
        states[max(0, s1_positions[i]):min(upper_bound, states.shape[0] - 1) ] = 1

    for i in range(s2_positions.shape[0]):
        lower_bound = max(s2_positions[i] - np.floor((mean_S2 + std_S2)), 0)
        upper_bound = min(states.shape[0], np.ceil(s2_positions[i] + np.floor(mean_S2) + std_S2))
        ...
        s2_index = np.min(states.shape[0], lower_bound + s2_index -1)
        upper_bound = min(states.shape[0], np.ceil(s2_index + (mean_S2)/2))
        states[np.max(S2_index - (mean_S2/2), 0): upper_bound] = 3
        ...
        if i <= s2_positions.shape[0]-1:
            ...
            states[np.ceil(s2_index) + (mean_S2):end_pos]
